...
import 'custom_underline_indicator.dart';
...
///  * [TabBar], which displays a row of tabs.
///  * [TabBarView], which displays a widget for the currently selected tab.
///  * [CustomTabController], which coordinates tab selection between a [TabBar] and a [TabBarView].
...
class CustomTab extends StatelessWidget {
  ...
  Widget _buildLabelText(BuildContext context) {
    final scale = DefaultTextStyle.of(context).style.fontSize / unselectedFontSize;
    final Size size = (TextPainter(
      text: TextSpan(
        text: text,
        style: TextStyle(fontSize: unselectedFontSize),
      ),
      maxLines: 1,
    ...
    return child ??
        Stack(
          ...
          children: [
            ...
            Text(text, style: TextStyle(fontSize: unselectedFontSize), softWrap: false, overflow: TextOverflow.fade),
          ],
        );
  }
  ...
  Widget _buildIcon(BuildContext context) {
    ...
    final Size size = (TextPainter(
      ...
      textDirection: TextDirection.ltr,
        ...
        .size;
    return child ?? Transform.scale(scale: 1 / scale, child: icon);
  }
...
class _CustomTabStyle extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle defaultStyle =
        (labelStyle ?? tabBarTheme.labelStyle ?? themeData.primaryTextTheme.body2).copyWith(inherit: true);
    final TextStyle defaultUnselectedStyle =
        (unselectedLabelStyle ?? tabBarTheme.unselectedLabelStyle ?? labelStyle ?? themeData.primaryTextTheme.body2)
            .copyWith(inherit: true);
    final TextStyle textStyle = selected
        ? TextStyle.lerp(defaultStyle, defaultUnselectedStyle, animation.value)
        : TextStyle.lerp(defaultUnselectedStyle, defaultStyle, animation.value);

    final Color selectedColor = labelColor ?? tabBarTheme.labelColor ?? themeData.primaryTextTheme.body2.color;
...
// upon layout. The tab widths are only used at paint time (see _CustomIndicatorPainter)
...
double _indexChangeProgress(CustomTabController controller) {
...
class CustomTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  final ValueChanged<int> onTapBefore;
  ...
  /// even if the tap doesn't change the CustomTabController's index. TabBar [onTap]
...
class _CustomTabBarState extends State<CustomTabBar> {
  ...
  void initState() {
    ...
    // the width of tab widget i. See _CustomIndicatorPainter.indicatorRect().
  ...
  void _updateTabController() {
    final CustomTabController newController = widget.controller;
    assert(() {
      if (newController == null) {
        throw FlutterError('No CustomTabController for ${widget.runtimeType}.\n'
  ...
  Future<void> _warpToCurrentIndex() async {
    ...
    if (mounted) {
      setState(() {
        ...
        if (_children[initialPage].key is GlobalKey) _children[previousIndex] = temp;
      });
    }
...
class CustomTabPageSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final CustomTabController tabController = controller;
    assert(() {
